...
**Date:** 17 Juni 2026  
**Author:** Giel × Claude  
**Status:** Locked — + strategi adopsi repo (License & Borrow Policy, Section 9)
...
Tools menjawab "apa yang chart tunjukkan." Giel menjawab "apa artinya, dan apa yang harus dipercaya." 4 Persona (GEMA, LEON, AKELA, RIVAN) adalah **framework berpikir Giel sendiri** saat membaca berita — diisi manual, bukan AI agent yang jalan otomatis. Kalau dipakai di konten, itu mensimulasikan cara Giel memecah analisa, bukan keluaran bot.
...
- **Context Engine:**
  ...
  - *Optional (tercatat, aktif saat scaling):* BTC L/S ratio + liquidation (Coinglass), stablecoin supply, HY credit spread, BI–Fed spread
...
IHSG     → SBN foreign flow, IDR, BI rate, komoditas
...
btc_long_short_ratio FLOAT      (optional — Coinglass)
btc_liquidation_24h  FLOAT      (optional — Coinglass)
stablecoin_supply    FLOAT      (optional — dry powder proxy)
hy_credit_spread     FLOAT      (optional — risk-off early warning, FRED)
...
└── SBN foreign flow → DJPPR / data Indonesia
```
...
├── driver            (net_liquidity / us10y / dxy / sbn_flow / etf_flow / dll)
...
IHSG  → sbn_foreign_flow=HIGH, usd_idr=HIGH, bi_rate=HIGH, komoditas=MED
...
- **Positioning (Layer C):** COT net long BTC/DXY/Gold, ETF flow, SBN foreign flow — arah uang besar
...
**Tujuan:** Tempat Giel menulis analisa sendiri, dengan 4 lensa sebagai struktur berpikir  
**Isi:** 4 kolom berlabel GEMA / LEON / AKELA / RIVAN — kosong, Giel isi manual saat baca news/data. Kolom opsional "External AI Check" untuk catat & banding hasil dari TradingAgents/qrak/LLM_trader/lainnya — diadu, bukan diikuti otomatis. Area "Conflict Notes" bebas untuk catat poin yang Giel sendiri belum sepakat antar lensa. Tidak ada tombol generate — pure input manual, tersimpan ke `reading_workspace`.
...
├── Copy/modif code ke repo Kastara → derivative work → lisariknya menular
...
```
═══════════════════════════════════════════════════════
  FOUNDATION — bangun mesin sekali (instrument = BTC)
═══════════════════════════════════════════════════════
 
PHASE A — DATABASE & SCRAPER
[ ] Setup SQLite schema (semua tabel: daily_market, asset_ohlcv,
...
[ ] Scraper: CoinGecko + Binance (BTC → asset_ohlcv + makro crypto)
[ ] Scraper: FRED (DXY, US10Y, VIX, WALCL/RRP/TGA → net liquidity, HY spread)
[ ] Scraper: yfinance (S&P500, Gold, USD/IDR, USD/JPY → context)
[ ] Scraper: Alternative.me (Fear & Greed)
[ ] Scraper: Economic Calendar (ForexFactory/Trading Economics)
[ ] RSS parser: news ingestion + rule-based impact scoring
[ ] Manual fetch tool (filter instrument + date range) — PRIORITAS,
    bootstrap data historis sebelum sistem live (lihat 4.1)
[ ] Scheduler: cron job lokal 00.00 WIB (VPS nanti)
 
PHASE B — ANALYSIS ENGINE (universal, di-test di BTC dulu)
[ ] Indicator calculator (MA, volume ratio) — baca asset_ohlcv
[ ] S&R zone detector (semi-otomatis + flag validasi manual)
[ ] Breakout + retest signal detector
[ ] R:R calculator + suggest entry/SL/TP1 → trade_signals
[ ] Form input manual_articles + trading_journal
[ ] Seed asset_context_weight untuk BTC
 
PHASE C — DASHBOARD (6 panel, instrument = BTC)
[ ] Backend API (Flask/FastAPI)
[ ] Panel 1: Data Snapshot + tombol Manual Backfill
[ ] Panel 2: News Briefing + tombol Add Manual Article
[ ] Panel 3: Forward Panel (Expectations + Positioning + Policy + Calendar)
[ ] Panel 4: Reading Workspace (4 lensa manual + external AI check)
[ ] Panel 5: Chart BTC + context chart + Approve/Reject signal
[ ] Panel 6: Synthesis + prediction_log capture + skor prediksi
 
PHASE D — FORWARD-LOOKING LAYER (bertahap, lihat 4.2)
[ ] Stage 1 — Layer B: CME FedWatch + Dot Plot manual → expectations
[ ] Stage 2 — Layer C: COT (CFTC) + ETF flow + SBN flow → positioning
[ ] Stage 3 — Layer A: policy_tracker + disiplin literal vs inferensi
 
PHASE E — TELEGRAM (jendela baca dari luar)
[ ] Telegram bot: push daily summary (Forward + signal + prediksi)
[ ] Telegram sebagai interface baca utama saat di luar
[ ] (Opsional Phase 2) format konten untuk publish X/IG
 
═══════════════════════════════════════════════════════
  EXPANSION — replikasi ke aset lain (pipeline sama)
  Tiap phase = ulangi pola: tambah data + context weight + chart
═══════════════════════════════════════════════════════
 
PHASE F — GOLD
[ ] Pastikan asset_ohlcv punya data Gold (sudah di-scrape sejak Phase A)
[ ] Seed asset_context_weight Gold (real yield, DXY, geopolitik)
[ ] Tambah chart Gold + context view di dashboard
[ ] Validasi engine S&R/breakout jalan benar di Gold
 
PHASE G — IHSG
[ ] Data IHSG + tambah driver lokal (SBN foreign flow, IDR, BI rate)
[ ] Seed asset_context_weight IHSG
[ ] Chart + context view IHSG
 
PHASE H — S&P 500
[ ] Context weight S&P (earnings, Fed path, net liquidity)
[ ] Chart + context view
 
PHASE I — FOREX (USD/IDR, USD/JPY)
[ ] Context weight forex (rate differential, carry, trade balance)
[ ] Chart + context view
...
Semua open question awal sudah terjawab. Sistem siap masuk Phase A.
 
---
 
*Kastara Finance Master Plan v1.3 — multi-aset architecture locked, build di-split per phase (Foundation A–E untuk BTC, Expansion F+ untuk aset lain). Living document.*